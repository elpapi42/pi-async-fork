...
export default function register(pi: any): void {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      triggerTurn: Type.Optional(Type.Boolean({ description: TRIGGER_TURN_DESCRIPTION })),
    }),
    renderCall: renderCreateForkCall,
    renderResult: renderCreateForkResult,
    async execute(toolCallId: string, params: { name: string; task: string; description: string; effort?: Tier; triggerTurn?: boolean }, signal: AbortSignal, _onUpdate: any, ctx: any) {
      const forkId = await getController(ctx).create(ctx, toolCallId, params.name, params.task, params.description, params.effort ?? "balanced", signal, params.triggerTurn ?? false);
