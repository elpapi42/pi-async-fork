...
- `forks/task-prompt.ts` appends each fork's identity, bounded-worker instructions, task, and required `Output` and `Learnings` report contract as the final user prompt.
