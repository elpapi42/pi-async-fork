...
- `forks/task-prompt.ts` owns the synthetic assistant boundary text, including identity, inherited-context framing, bounded-worker instructions, milestone-report protocol, and the full `Output` and `Learnings` report contract. It also owns the assigned-task user message, its evidence-, state-, novelty-, and compactness-gated progress-report requirement, and its concise final-response format requirement.
...
6. `create_fork` creates the child session and fleet agent, starts reception, sends the assigned task followed by the evidence-, state-, novelty-, and compactness-gated progress-report requirement and concise final-response format requirement as user content, receives initial-task acceptance, appends `fork.created`, and returns before the child completes.
