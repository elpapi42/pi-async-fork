...
Each visible checkpoint and final report uses `## Output` and `## Learnings`. Intermediate reports contain at most 180 words, three findings, and two learning bullets. Source URLs can appear inline. Final reports retain the task-adapted report contract. To continue one Pi run, an intermediate report must include the next necessary tool call in the same assistant response. A text-only report with no next tool call is final.
